...
**airgap-pkg** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `airgap-pkg` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
airgap-pkg ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
airgap-pkg ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
airgap-pkg ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `airgap-pkg` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="airgap-pkg") for rec in records]   # records = your JSON output
